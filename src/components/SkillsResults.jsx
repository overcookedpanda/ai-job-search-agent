  ...
  return (
    <div className="bg-white dark:bg-gray-800 rounded-xl shadow-lg p-6 border border-gray-100 dark:border-gray-700">
      <div className="mb-6 text-center">
        ...
        {skills.company && (
          <p className="text-lg text-gray-600 dark:text-gray-300">{skills.company}</p>
        )}
        {skills.location && (
          <span className="inline-block bg-blue-100 dark:bg-blue-900 text-blue-800 dark:text-blue-200 text-sm font-medium mr-2 px-2.5 py-0.5 rounded mt-2">
            {skills.location}
          </span>
        )}
        {skills.salary && (
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <ul className="space-y-2">
              {skills.required_skills.technical_skills.map((skill, index) => (
                <li key={index} className="flex items-start text-left">
                  <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5 mr-2 text-green-500 dark:text-green-400 mt-0.5 flex-shrink-0" viewBox="0 0 20 20" fill="currentColor">
        ...
        {hasContent(skills.required_skills?.experience) && (
          <div className="bg-gray-50 dark:bg-gray-700 p-4 rounded-lg">
            <h3 className="text-lg font-semibold text-gray-800 dark:text-white mb-3 flex items-center">
              <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5 mr-2 text-yellow-600 dark:text-yellow-400" viewBox="0 0 20 20" fill="currentColor">
            ...
            <ul className="space-y-2">
              {skills.required_skills.experience.map((exp, index) => (
                <li key={index} className="flex items-start text-left">
                  ...
                  <span className="text-gray-700 dark:text-gray-200">{exp}</span>
                </li>
              ))}
            </ul>
          </div>
            ...
            <ul className="space-y-2">
              {skills.required_skills.education.map((edu, index) => (
                <li key={index} className="flex items-start text-left">
                  ...
                  <span className="text-gray-700 dark:text-gray-200">{edu}</span>
                </li>
              ))}
            </ul>
          </div>
          ...
          <div className="bg-gray-50 dark:bg-gray-700 p-4 rounded-lg">
            ...
            <ul className="space-y-2">
              {skills.preferred_skills.map((skill, index) => (
                <li key={index} className="flex items-start text-left">
                  <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5 mr-2 text-blue-500 dark:text-blue-400 mt-0.5 flex-shrink-0" viewBox="0 0 20 20" fill="currentColor">
